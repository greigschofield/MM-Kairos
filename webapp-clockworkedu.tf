...
#change clockworkedu (find and replace)
#change the service plan ID to match which web app service plan you want it to sit on... (line 21)

#create new file

resource "azurerm_windows_web_app" "azmmclientrg-webbapp-clockworkedu" {
  app_settings = {
    ...
    MM_DEFAULT_VERSION                         = "mmakerclockworkedu"
  ...
  name                      = "clockworkedu-matchmakersoftware"
...
/**
resource "azurerm_app_service_custom_hostname_binding" "azmmclientrg-custombinding-webapp-clockworkedu1" {
  app_service_name    = "clockworkedu-matchmakersoftware"
  hostname            = "clockworkedu-matchmakersoftware.azurewebsites.net"
  resource_group_name = "azmmclientapps"
}
*/

resource "azurerm_app_service_custom_hostname_binding" "azmmclientrg-custombinding-webapp-clockworkedu2" {
  app_service_name    = "clockworkedu-matchmakersoftware"
  hostname            = "clockworkedu.matchmakersoftware.com"
  resource_group_name = "azmmclientapps"
  
}

resource "azurerm_app_service_certificate_binding" "azmmclientrg-custombinding-cert-webapp-clockworkedu2" {
  hostname_binding_id = azurerm_app_service_custom_hostname_binding.azmmclientrg-custombinding-webapp-clockworkedu2.id
  certificate_id      = azurerm_app_service_certificate.azmmclientrg-365.id
  ssl_state           = "SniEnabled"
}


resource "azurerm_monitor_smart_detector_alert_rule" "azmmclientrg-smartalert-webapp-clockworkedu" {
  description         = "Failure Anomalies notifies you of an unusual rise in the rate of failed HTTP requests or dependency calls."
  detector_type       = "FailureAnomaliesDetector"
  frequency           = "PT1M"
  name                = "Failure Anomalies - clockworkedu-matchmakersoftware"
  resource_group_name = "AZMMClientApps"
  scope_resource_ids  = ["/subscriptions/2bd29ed6-34a6-42d7-8235-293e8ee67447/resourcegroups/azmmclientapps/providers/microsoft.insights/components/clockworkedu-matchmakersoftware"]
  severity            = "Sev3"
  action_group {
...
resource "azurerm_application_insights" "azmmclientrg-appinsight-webapp-clockworkedu" {
