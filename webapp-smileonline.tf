...
resource "azurerm_windows_web_app" "azmmclientrg-webbapp-smile-online" {
  app_settings = {
...
/**
resource "azurerm_app_service_custom_hostname_binding" "azmmclientrg-custombinding-webapp-smile-online1" {
  app_service_name    = "smile-online-matchmakersoftware"
  hostname            = "smile-online-matchmakersoftware.azurewebsites.net"
  resource_group_name = "azmmclientapps"
}
*/
...
resource "azurerm_app_service_certificate_binding" "azmmclientrg-custombinding-cert-webapp-smile-online2" {
  hostname_binding_id = azurerm_app_service_custom_hostname_binding.azmmclientrg-custombinding-webapp-smile-online2.id
  certificate_id      = azurerm_app_service_certificate.azmmclientrg-365.id
  ssl_state           = "SniEnabled"
}


resource "azurerm_monitor_smart_detector_alert_rule" "azmmclientrg-smartalert-webapp-smile-online" {
  description         = "Failure Anomalies notifies you of an unusual rise in the rate of failed HTTP requests or dependency calls."
  detector_type       = "FailureAnomaliesDetector"
  frequency           = "PT1M"
  name                = "Failure Anomalies - smile-online-matchmakersoftware"
  resource_group_name = "AZMMClientApps"
  scope_resource_ids  = ["/subscriptions/2bd29ed6-34a6-42d7-8235-293e8ee67447/resourcegroups/azmmclientapps/providers/microsoft.insights/components/smile-online-matchmakersoftware"]
  severity            = "Sev3"
  action_group {
...
resource "azurerm_application_insights" "azmmclientrg-appinsight-webapp-smile-online" {
