...
#change HIE (find and replace)
#change the service plan ID to match which web app service plan you want it to sit on...

#create new file

resource "azurerm_windows_web_app" "azmmclientrg-webbapp-HIE" {
  app_settings = {
...
/**
resource "azurerm_app_service_custom_hostname_binding" "azmmclientrg-custombinding-webapp-HIE1" {
  app_service_name    = "HIE-matchmakersoftware"
  hostname            = "HIE-matchmakersoftware.azurewebsites.net"
  resource_group_name = "azmmclientapps"
}
*/

resource "azurerm_app_service_custom_hostname_binding" "azmmclientrg-custombinding-webapp-HIE2" {
  app_service_name    = "HIE-matchmakersoftware"
  hostname            = "HIE.matchmakersoftware.com"
  resource_group_name = "azmmclientapps"
  
}

resource "azurerm_app_service_certificate_binding" "azmmclientrg-custombinding-cert-webapp-HIE2" {
  hostname_binding_id = azurerm_app_service_custom_hostname_binding.azmmclientrg-custombinding-webapp-HIE2.id
  certificate_id      = azurerm_app_service_certificate.azmmclientrg-365.id
  ssl_state           = "SniEnabled"
}


resource "azurerm_monitor_smart_detector_alert_rule" "azmmclientrg-smartalert-webapp-HIE" {
  description         = "Failure Anomalies notifies you of an unusual rise in the rate of failed HTTP requests or dependency calls."
  detector_type       = "FailureAnomaliesDetector"
  frequency           = "PT1M"
  name                = "Failure Anomalies - HIE-matchmakersoftware"
  resource_group_name = "AZMMClientApps"
  scope_resource_ids  = ["/subscriptions/2bd29ed6-34a6-42d7-8235-293e8ee67447/resourcegroups/azmmclientapps/providers/microsoft.insights/components/HIE-matchmakersoftware"]
  severity            = "Sev3"
  action_group {
...
resource "azurerm_application_insights" "azmmclientrg-appinsight-webapp-HIE" {
